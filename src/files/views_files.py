...
from src.files.service import add_new_file_and_save_od_disk, get_file_metadata_by_id, delete_file_by_id, \
    get_file_children, change_file_content_on_disk, update_file
...
async def update_file(file_meta: FileMetadataUpdate, db: Session = Depends(get_db),
    ...
    try:
        file_ = update_file(db, file_meta, current_user.id)
    except FileNotFoundException as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    return file_
...
async def change_file_content(file_id: int, file_data: UploadFile, db: Session = Depends(get_db),
                              current_user: User = Depends(get_current_user)):
    try:
        file_ = change_file_content_on_disk(db, file_id, file_data, current_user.id)
    except FileNotFoundException as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    return file_
...
async def get_directory_children(file_id: int, db: Session = Depends(get_db),
                                 current_user: User = Depends(get_current_user)):
    return get_file_children(db, file_id, current_user.id)
